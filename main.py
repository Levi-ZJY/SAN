...
from dataset import ImageDataset, TextDataset, TextRadicalDataset
...
def _get_radical_language_databaunch(config):
    kwargs = {
        'max_length': config.dataset_max_length_radical,
        'case_sensitive': config.dataset_case_sensitive,
        'charset_path': config.dataset_charset_path_radical,
        'smooth_label': config.dataset_smooth_label,
        'smooth_factor': config.dataset_smooth_factor,
        'one_hot_y': config.dataset_one_hot_y,
        'use_sm': config.dataset_use_sm,
    }
    train_ds = TextRadicalDataset(config.dataset_train_roots[0], is_training=True, **kwargs)
    valid_ds = TextRadicalDataset(config.dataset_test_roots[0], is_training=False, **kwargs)
    data = DataBunch.create(
        path=train_ds.path,
        train_ds=train_ds,
        valid_ds=valid_ds,
...
def _get_learner(config, data, model, local_rank=None):
    ...
    if config.global_stage == 'pretrain-language':
    ...
    elif config.global_stage == 'pretrain-radical-language':
        metrics = [TopKTextAccuracy(
            k=ifnone(config.model_k, 5),
            charset_path=config.dataset_charset_path_radical,
            max_length=config.dataset_max_length_radical + 1,
            case_sensitive=config.dataset_eval_case_sensisitves,
            model_eval=config.model_eval)] 
    else:
        metrics = [TextAccuracy(
            ...
            model_eval=config.model_eval,
            #!
            charset_path_radical=config.dataset_charset_path_radical,
            max_length_radical=config.dataset_max_length_radical + 1,
            global_phase=config.global_phase
    ...
    if config.global_phase == 'train':
        ...
        learner.callbacks += [
            DumpPrediction(learn=learner,
                    ...
                    #!
                    charset_path_radical=config.dataset_charset_path_radical,
                    max_length_radical=config.dataset_max_length_radical,
...
def main():
    ...
    elif config.global_stage == 'pretrain-radical-language': data = _get_radical_language_databaunch(config)
    else: data = _get_databaunch(config)
    
    logging.info('Construct model.(main)')
