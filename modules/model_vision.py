...
class BaseVision(Model):
    def __init__(self, config):
        ...
        if config.model_vision_attention == 'position':                   
            ...
            #!
            self.attentionRadical = PositionAttention(
                max_length=config.dataset_max_length_radical + 1,  # additional stop token
                mode=mode,
            )
        ...
        #!
        self.clsRadical = nn.Linear(self.out_channels, self.charset_radical.num_classes)
    ...
    def forward(self, images, *args):
        ...
        #！
        attn_vecs_radical, attn_scores_radical = self.attentionRadical(features)
        
        logits_radical = self.clsRadical(attn_vecs_radical)   
        pt_lengths_radical = self._get_length(logits_radical) 
        

        return {'feature': attn_vecs, 'feature_radical': attn_vecs_radical,
                'logits': logits, 'pt_lengths': pt_lengths,'attn_scores': attn_scores, 
                'logits_radical': logits_radical, 'pt_lengths_radical': pt_lengths_radical,'attn_vecs_radical': attn_vecs_radical,
                'loss_weight':self.loss_weight, 'name': 'vision'}
