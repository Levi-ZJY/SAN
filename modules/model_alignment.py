...
class BaseAlignmentRadical(Model):
    def __init__(self, config):
        super().__init__(config)
        d_model = ifnone(config.model_alignment_d_model, _default_tfmer_cfg['d_model'])
        ...
        self.w_att = nn.Linear(2 * d_model, d_model)
        self.cls_radical = nn.Linear(d_model, self.charset_radical.num_classes)
    ...
    def forward(self, l_feature, v_feature):
        ...
        logits = self.cls_radical(output)  # (N, T, C)
        pt_lengths = self._get_length(logits)

        return {'logits_radical': logits, 'pt_lengths_radical': pt_lengths, 'loss_weight':self.loss_weight,
                'name': 'alignment', 'name_radical': 'alignment_radical'}   #!