...
class BCNRadicalLanguage(Model):
    def __init__(self, config):
        ...
        self.proj = nn.Linear(self.charset_radical.num_classes, d_model, False)
        self.token_encoder = PositionalEncoding(d_model, max_len=self.max_length_radical)
        self.pos_encoder = PositionalEncoding(d_model, dropout=0, max_len=self.max_length_radical)
        ...
        self.cls = nn.Linear(d_model, self.charset_radical.num_classes)

        if config.model_language_radical_checkpoint is not None:
            logging.info(f'Read language_radical model from {config.model_language_radical_checkpoint}.')
            self.load(config.model_language_radical_checkpoint)
    ...
    def forward(self, tokens, lengths):
        ...
        padding_mask = self._get_padding_mask(lengths, self.max_length_radical)

        zeros = embed.new_zeros(*embed.shape)
        qeury = self.pos_encoder(zeros)
        location_mask = self._get_location_mask(self.max_length_radical, tokens.device)
        ...
        res =  {'feature': output, 'logits_radical': logits, 'pt_lengths_radical': pt_lengths,
