...
class TextAccuracy(Callback):
    ...
    def __init__(self, charset_path, max_length, case_sensitive, model_eval, charset_path_radical, max_length_radical, global_phase):  #!
        ...
        #! 
        self.charset_path_radical = charset_path_radical
        self.max_length_radical = max_length_radical
        self.charset_radical = CharsetMapper(charset_path_radical, self.max_length_radical)
    ...
    def _get_output(self, last_output):
        if isinstance(last_output, (tuple, list)): 
            for res in last_output:
                if res['name'] == self.model_eval and 'name_radical' not in res.keys(): 
                    output = res
        else: output = last_output
        return output
    
    #! 
    def _get_output_radical(self, last_output):
        if isinstance(last_output, (tuple, list)): 
            for res in last_output:
                if 'name_radical' in res.keys():
                    if res['name_radical'] == 'alignment_radical': 
                        output_radical = res
        else: print('?????????????1')
        return output_radical
    ...
    #! 
    def _update_output_radical(self, last_output, items):
        if isinstance(last_output, (tuple, list)): 
            for res in last_output:
                if 'name_radical' in res.keys():
                    if res['name_radical'] == 'alignment_radical': 
    ...
    #! 
    def decode_radical(self, logits_radical):
        """ Greed decode """
        # TODO: test running time and decode on GPU
        out_radical = F.softmax(logits_radical, dim=2)
        pt_text_radical, pt_scores_radical, pt_lengths_radical = [], [], []
        for o in out_radical:
            text_radical = self.charset_radical.get_text(o.argmax(dim=1), padding=False, trim=False)
            text_radical = text_radical.split(self.charset_radical.null_char)[0]  # end at end-token
            pt_text_radical.append(text_radical)
            pt_scores_radical.append(o.max(dim=1)[0])
            pt_lengths_radical.append(min(len(text_radical) + 1, self.max_length_radical))  # one for end-token
        pt_scores_radical = torch.stack(pt_scores_radical)
        pt_lengths_radical = pt_scores_radical.new_tensor(pt_lengths_radical, dtype=torch.long)
        return pt_text_radical, pt_scores_radical, pt_lengths_radical
...
class TopKTextAccuracy(TextAccuracy):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        # 
        if 'logits' in last_output.keys():
            logits, pt_lengths = last_output['logits'], last_output['pt_lengths']
        else:
            logits, pt_lengths = last_output['logits_radical'], last_output['pt_lengths_radical']
        ...
        for logit, pt_length, label, length in zip(logits, pt_lengths, gt_labels, gt_lengths):
            ...
            for i in range(len(label)):
                ...
                #print('1',len(logit[i]),len(logit),char_logit)
                char_label = label[i].argmax(-1)
                #print('2',len(label[i]),len(label),char_label)
...
class DumpPrediction(LearnerCallback):
    ...
    def __init__(self, learn, dataset, charset_path, max_length, charset_path_radical, max_length_radical, model_eval, image_only=False, debug=False):
        ...
        self.charset = CharsetMapper(filename=charset_path, max_length=max_length)
        #!
        self.charset_radical = CharsetMapper(filename=charset_path_radical, max_length=max_length_radical)
    ...
    def on_batch_end(self, last_input, last_output, last_target, **kwargs):
        if isinstance(last_output, (tuple, list)):
            for res in last_output:
                if res['name'] == self.model_eval and 'name_radical' not in res.keys(): 
                    pt_text = res['pt_text']
                if res['name'] == 'vision': 
                    attn_scores = res['attn_scores'].detach().cpu()
                if res['name'] == self.model_eval and 'name_radical' not in res.keys(): 
                    logits = res['logits']
                if self.model_eval == 'alignment':
                    if 'name_radical' in res.keys():
                        if res['name_radical'] == 'alignment_radical':
                            pt_text_radical = res['pt_text_radical']
                            logits_radical = res['logits_radical']
        ...
        #!
        if self.model_eval == 'alignment':
            pt_text_radical = [self.charset_radical.trim(t) for t in pt_text_radical]
        """
        ...
        #!
        if self.model_eval == 'alignment':
            gt_text_radical = [self.charset_radical.conver_label_to_radical_label(text) for text in gt_text]
        ...
        for gt, pt, image, attn, logit in zip(gt_text, pt_text, images, attn_scores, logits):
            ...
            if not self.image_only:
                ...
                ##################################################################################################
                gtt=gt.replace(':','*1*').replace('/','*2*').replace('\\','*3*')
                ptt=pt.replace(':','*1*').replace('/','*2*').replace('\\','*3*')
                ##################################################################################################
                self.pil(image).save(self.attn_root/f'{self.c}_{gtt}_{ptt}.jpg')
            self.c += 1
