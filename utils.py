...
class CharsetMapper(object):
    ...
    def conver_label_to_radical_label(self,label_text):
        radical_label=""
        ...
        radical_label = radical_label.lower()
        return radical_label
    ...
    def get_labels(self, text, length=None, padding=True, case_sensitive=False):
        ...
        #
            ...
            for i in range(len(special)):
                specialnum.append(self.char_to_label[special[i]])
            #
            text_spl=[]
            for i in range(len(special)):
                if i==0:
            ...
            for i in range(len(text_spl)):
                for j in range(len(text_spl[i])):
                    num=self.char_to_label[text_spl[i][j]]
                    labels.append(num)
                if i!=len(text_spl)-1:
                    labels.append(specialnum[i])
        """with open("/root/data/my-zh-ABINet-main/ttt2.txt", "a") as file:
            if len(labels)!=34:
        ...
        def __dict2attr(d, prefix=''):
            for k, v in d.items():
                if isinstance(v, dict):
                    ...
                    if k == 'stage':
                        assert v in ['pretrain-vision', 'pretrain-language','pretrain-radical-language',
